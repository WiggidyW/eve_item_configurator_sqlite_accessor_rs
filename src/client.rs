...
impl Client {
    pub async fn new(
        ...
        let connect_options = SqliteConnectOptions::from_str(url.as_ref())
            .map_err(|e| Error::InvalidUrlError(e))?;
        let pool = match pool_options {
            Some(p) => p.connect_with(connect_options)
                .await
    ...
    pub (crate) async fn select_u32s(
        &self,
        sql: &'static str,
